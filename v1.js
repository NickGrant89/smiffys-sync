...
// Load product mapping from MongoDB (include last known stock levels and prices)
...
// Save product mapping to MongoDB (optimized to save only changes, include stock levels and prices)
...
// Fetch local image for a product
async function getLocalImage(baseProductCode) {
  const imagePath = path.join(IMAGE_DIR, `${baseProductCode}.jpg`);
  try {
    await fs.access(imagePath);
    const buffer = await fs.readFile(imagePath);
    return { path: imagePath, attachment: buffer.toString('base64') };
  } catch (error) {
    return null;
  }
}
...
// Transform Smiffys products to Shopify format
async function transformSmiffysToShopify(smiffysProducts) {
  ...
  for (const [groupingCode, productData] of finalEntries) {
    imagePromises.push(getLocalImage(groupingCode));
  ...
  shopifyProducts.forEach((shopifyProduct, index) => {
    if (images[index]) shopifyProduct.images = [{ attachment: images[index].attachment }];
  });
    ...
    try {
      const products = await withRetry(() =>
        shopify.product.list({
          ...
          fields: 'id,title,variants,vendor,product_type,tags,status'
        })
      );
      for (const product of products) {
        ...
        if (genericCodeTag) {
          ...
          productSyncLogger.info(`Found product for genericCode ${genericCode} in mapping: Shopify ID ${product.id}, Title: ${product.title}, Tags: ${product.tags}, Status: ${product.status}`);
        }
      }
        ...
        for (const product of products) {
          ...
          if (genericCodeTag) {
            ...
            if (unmappedGenericCodes.has(genericCode)) {
              ...
              productSyncLogger.info(`Found product for genericCode ${genericCode} via tag search: Shopify ID ${product.id}, Title: ${product.title}, Tags: ${product.tags}, Status: ${product.status}`);
              unmappedGenericCodes.delete(genericCode);
            }
          }
        }
...
// Sync a single product to Shopify (update stock and prices only if changed)
async function syncProduct(productData, existingProducts, globalProductMapping, failureLogger) {
  ...
  try {
    ...
    if (!cachedMapping) {
      ...
      productSyncLogger.info(`MongoDB mapping for ${baseSku}: Shopify ID ${cachedMapping.shopifyProductId}, Last Stock: ${JSON.stringify(cachedMapping.lastStock)}`);
    }
    ...
    if (existingProduct && !FORCE_CREATE) {
      ...
      // Include all existing variants in the update to preserve those not in the Smiffys feed
      for (const existingVariant of existingVariants) {
        ...
        updateData.variants.push(variantData);
      }

      // Update variants that are in the Smiffys feed and prepare inventory updates
      for (const newVariant of productData.variants) {
        ...
        if (existingVariant) {
          ...
          // Log stock comparison for 50725L
          if (newVariant.sku === '50725L') {
            productSyncLogger.info(`Stock comparison for ${baseSku} variant ${newVariant.sku}: Shopify stock = ${shopifyStock}, Smiffys stock = ${newStock}`);
            // Force stock update for 50725L to debug
            stockChanged = true;
          } else {
            // Compare stock directly with Shopify's current stock
          ...
          // Update the variant in the updateData (excluding inventory_quantity)
          const variantIndex = updateData.variants.findIndex(v => v.sku === newVariant.sku);
          if (variantIndex !== -1) {
            updateData.variants[variantIndex] = {
              id: existingVariant.id,
              sku: newVariant.sku,
              price: newPrice,
          ...
          // We can't set inventory until the variant is created, so we'll handle it after creation
          stockChanged = true;
          priceChanged = true;
        }
      }
      ...
      // Force update for 50725 to debug (remove after confirming fix)
      if (baseSku === '50725') {
        productSyncLogger.info(`Forcing update for product ${baseSku} to debug stock issue`);
        stockChanged = true;
      }
      ...
      productSyncLogger.info(`Updated product ${baseSku}: Vendor: ${syncedProduct.vendor}, Status: ${syncedProduct.status}, Tags: ${syncedProduct.tags}`);
      productSyncLogger.info(`Skipped collection update for existing product ${baseSku} (Shopify ID ${existingProduct.id})`);
      ...
      try {
        ...
        await productMappingCollection.updateOne(
          { genericCode: baseSku },
          { $set: { genericCode: baseSku, shopifyProductId: 'PENDING', lastStock: {}, lastPrices: {}, createdAt: new Date() } },
          { upsert: true }
        );
      ...
      try {
        ...
        productSyncLogger.info(`Created ${baseSku}: ${productData.title} (Type: ${productData.product_type}, Tags: ${productData.tags}, Vendor: ${productData.vendor}, Status: ${syncedProduct.status})`);
        const stockMap = {};
        const priceMap = {};
        productData.variants.forEach(variant => {
          stockMap[variant.sku] = variant.inventory_quantity;
        ...
        await productMappingCollection.updateOne(
          { genericCode: baseSku },
          { $set: { shopifyProductId: syncedProduct.id, lastStock: stockMap, lastPrices: priceMap, updatedAt: new Date() } }
        );
